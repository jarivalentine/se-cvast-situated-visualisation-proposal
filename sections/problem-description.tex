...
Situated visualization is most definitely part of the ongoing trend toward virtual and augmented reality.
VR technology has been growing steadily, improving in quality, and expanding beyond entertainment as the desire to use it for more purposeful applications becomes strong.

\subsection{Status Quo}

Bressa et al. (2022) worked on an extensive list of papers to conclude perspectives on situated visualization \cite{BKT+22} and found that the most broad definition is by White and Feiner (2009). They state situated visualization ``is related to and displayed in its environment'' \cite{WF09}.
They also found Willett et al. (2017) to build on that work introducing concepts like the ``physical data referen'' as mentioned previously \cite{WYP17}.
...
Are the same definitions used outside of research (where we will find exactly the type of approaches which might be insightful)?
...
Beyond the term ``situated visualization'', we could argue that the concept dates back to the 1970s, when te U.S. Department of Defense started operating their GPS system (alongside the USSR's GLONASS). This was emphasized by Spiekermann back in 2004, when the term ``location-based services'' was used to describe services that integrate a mobile device's location with other data to provide value to a user \cite{SV04}.
...
\end{enumerate}
Providing answers to these questions help the users make more informed decisions, by encouraging them to reflect on certain aspects that might otherwise be overlooked.
