...
Beyond the term ``situated visualization'', we could argue that the concept dates back to the 1970, when te U.S. Department of Defense started operating their GPS system (alongside the USSR's GLONASS). This was emphasized by Spiekermann back in 2004, when the term ``location-based services'' was used to describe services that integrate a mobile device's location with other data to provide value to a user \cite{SV04}.
...
\end{enumerate}
Proving answers to these questions help the users make more informed decisions by encouraging them to reflect on certain aspects that might otherwise be overlooked.
